...
class ScreenTask22(QMainWindow):
    def __init__(self, name, identifier, tasks):
        ...
        self.nr = "22" # task number, first digit = task second digit = hard/easy 1 = easy 2 = hard
    ...
    def next_page(self):
        ...
        if self.input_answer.currentText() != '':
            with open("results/" + self.identifier + self.nr + "2", 'w', newline='') as myfile:
                ...
                wr.writerows(self.input_answer.currentText())
...
def main():
    ...
    info = ScreenTask22("Max Mustermann", "1234", [])
    sys.exit(app.exec_())
